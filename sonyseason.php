...
$response = Requests::get('https://apiv2.sonyliv.com/AGL/2.3/A/ENG/WEB/IN/DL/DETAIL/'.$_GET['id'].'?kids_safe=false&from=0&to=19', $headers);
...
foreach ($data1 as $value) {
	$episodeCount = "";
	if(empty($value['episodeCount'])){
		$episodeCount = "";
	} else {
		$episodeCount = $value['episodeCount'];
	}
	$tv_background_image = "";
	if(empty($value['metadata']['emfAttributes']['tv_background_image'])){
		$tv_background_image = $value['metadata']['emfAttributes']['thumbnail'];
	} else {
		$tv_background_image = $value['metadata']['emfAttributes']['tv_background_image'];
	}
	$json[] = array(
